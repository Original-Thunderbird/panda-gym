...
class Push(Task):
    ...
    def compute_reward(self, achieved_goal, desired_goal, init_obs, observations, info: Dict[str, Any]) -> np.ndarray:
        ...
        d = -obj_arr * dst_arr * distance(observations[0:3], init_obs[6:9]) * max(abs(distance(observations[0:3], observations[6:9]) - 0.1), 0) + distance(observations[0:3], desired_goal) ** 2
